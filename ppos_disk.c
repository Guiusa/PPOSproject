...
void diskDriverBody (void *args){
    for(;;){
        ...
        #ifdef DEBUG
            printf("[DiskDriverBody]\tstatus: %d\n\t\t\tqueue size: %d\n",
                disk_cmd(DISK_CMD_STATUS, 0, 0),
                queue_size((queue_t *) disk_components.reqs)
                ) ;
        #endif
...
int disk_block_read (int block, void *buffer) {
    ...
    r->buff = buffer ;
    #ifdef DEBUG
        printf("[disk_block_read]\tRequisição de %s registrada para bloco %d\n",
...
int disk_block_write (int block, void *buffer) {
    ...
    r->buff = buffer ;
    #ifdef DEBUG
        printf("[disk_block_write]\tRequisição de %s registrada para bloco %d\n",
            (r->op) ? "leitura" : "escrita",
            r->blk
        ) ;
    #endif
